...
>(
  type: KW | KH,
  listenerCallback: (event: WindowEventMap[KW] | HTMLElementEventMap[KH] | Event) => void,
  target?: RefObject<T>
  ...
  useEffect(() => {
    ...
    const eventListenerCallback: typeof listenerCallback = (event) => savedHandler.current(event);
